$.fn.bindUpload = function () {
    var f = {
        ...
        inputName: arguments[0].inputName || 'pics[]',
        callback: arguments[0].callback || '',
        btn: arguments[0].btn || '#upload-btn',
    };
    var g = this;
    var c = $(f.btn).find("input.add");
    var e = 0;
    var a = null;
    var b = [];
    var d = /[\u4e00-\u9fa5]/g;
    ...
    this.openWindow = function (k) {
        ...
        //+ ((i / 1048576).toFixed(3) == 0 ? i / 1048576 : (i / 1048576).toFixed(3)) +
        var file_length = b.length > f.num ? f.num : b.length;
        if (b.length > f.num) {
            alert('最多只能传' + f.num + '张图片');
            return false;
        }
        var j = '<div id="zwb_upload_status"><div class="box">        <div class="title">            共'
            + file_length +
            '个文件  </div>        ' +
            '<div class="list"><ul class="listBox"></ul></div> ' +
            '       <div class="foot">        ' +
    ...
    this.getImgSrc = function (l) {
        ...
        if (k == "png" || k == "jpg" || k == "gif" || k == "svg") {
            ...
            h.onload = function () {
                var o = h.result;
                if (e < f.num || f.num == "") {
                    g.find("#zwb_upload_status ul.listBox").append('<li><img  src="' + o + '"/><a><p class="name">' + i +
                        '</p><p class="size">大小: ' + ((j / 1048576).toFixed(3) == 0 ? j / 1048576 : (j / 1048576).toFixed(
                            3)) +
                        'b</p><p>状态: <b class="state">未上传</b></p></a><span class="success"></span><span class="delete"></span><div class="err"' +
                        (m ? 'style="width:100%;"' : "") + ">" + m +
                        '</div><div class="progressBox"><div class="progress"></div></div>' +
                        '<input type="hidden" class="upload_file" value="" name="' + f.inputName + '" >' +
                        '</li>')
                }
                e++;
                if (e == l.length) {
            ...
            if (e < f.num || f.num == "") {
                g.find("#zwb_upload_status ul.listBox").append('<li><img  src="' + imgPath + '"/><a><p class="name">' +
                    i + '</p><p class="size">大小: ' + ((j / 1048576).toFixed(3) == 0 ? j / 1048576 : (j / 1048576).toFixed(
                        3)) +
                    'b</p><p>状态: <b class="state">未上传</b></p></a><span class="success"></span><span class="delete"></span><div class="err"' +
                    (m ? 'style="width:100%;"' : "") + ">" + m +
