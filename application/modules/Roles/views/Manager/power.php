                    ...
                    <tr class="<?php if ( $i % 2 == 0 ): ?>tr1<?php endif; ?>">
                        <td width="22%" height="35" align="right">
                            <p class="b">
                                ...
                                (
                                <a href="javascript:;" href="javascript:;" parent="<?= $mr['id'] ?>"
                                   onclick="checkPrivileges('<?php echo $v['id'] ?>','on', this)">全选</a> /
                                <a href="javascript:;" href="javascript:;" parent="<?= $mr['id'] ?>"
                                   onclick="checkPrivileges('<?php echo $v['id'] ?>','off', this)">反选</a>
                                )：
                            </p>
                        </td>
                        <td colspan="2" align="left" id="<?= $v['id'] ?>_p">
                            ...
                            <label for="p_item_<?= $v['id'] ?>" class="labels" style="cursor: pointer;">
                                <input id="p_item_<?= $v['id'] ?>" type="checkbox"
                                       onclick="checkPrivilegesParent(<?php echo $v['parent']['id'] ?>,this)"
                                       name="privileges[<?php echo $v['id'] ?>][]"
                            ...
                            <?php foreach ( $v['submenu'] as $methodk => $methodv ): ?>
                                ...
                                ?>
                                <label class="labels" for="p_item_<?= $methodv['id'] ?>"
                                       style="font-weight:normal;cursor: pointer;">
                                    <input id="p_item_<?= $methodv['id'] ?>"
                                           onclick="checkPrivilegesParent(<?php echo $v['parent']['id'] ?>,this)"
                                           type="checkbox" name="privileges[<?php echo $methodv['id'] ?>][]"
...
<script>
    ...
    function checkPrivileges(id, b, obj) {
        var checkd = b == 'on' ? true : false;
        ...
        if(checkLen <=0 && b== 'off'){
            $('#parent_input_' + parent).prop('checked', checkd);
        }else{
    ...
    function checkALLPrivileges2(id, b) {
        var checkd = b == 'on' ? true : false;
        $('#' + id + '_p').find('input').prop('checked', checkd);
        $('#parent_input_' + id).prop('checked', checkd);
